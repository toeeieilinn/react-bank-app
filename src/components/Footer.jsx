...
const SocialMedia = ({ icon, name, link }) => {
  return (
    <a href={link} className="relative social-icon">
      <span className="social-media-icon mt-8 ss:mt-0 p-3 xs:py-4 ss:px-[1.03rem] text-black block rounded-full z-2 ss:absolute top-1/2 left-0 -translate-y-1/2">
        {icon}
      </span>
    ...
    <section className="relative font-poppins pb-10 sm:pb-16">
      ...
      <div className="flex items-center justify-between md:flex-row flex-col gap-5 pt-10 mt-10 sm:mt-16 border-t">
        <p className="md:text-base xs:text-sm text-xs">
          Copyright @ {date} McNamee. All Rights Reserved.
        </p>
        <div>
          <div className="flex items-center gap-3 xs:gap-5 text-lg xs:text-xl">
            <SocialMedia icon={<SiGithub />} name="github" link="" />
            <SocialMedia icon={<SiLinkedin />} name="linkedin" link="" />
            <SocialMedia icon={<TbBrandFiverr />} name="fiverr" link="" />
            <SocialMedia icon={<SiUpwork />} name="upWork" link="" />
          </div>
        </div>
      </div>
    </section>
